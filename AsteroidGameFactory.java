...
public class AsteroidGameFactory implements IAsteroidGameFactory
{
	...
	public BoardComponent MakeSheild(Square square ,int SheildHealth){

		SheildClassDecorator sheildClassDecorator = new SheildClassDecorator(square);
		return sheildClassDecorator ;
	...
	public Command MakeCommand(String commandLine)
	{
		...
		switch (commandToMake.toUpperCase())
		{
			...
			case "SPAWN_SHIELD":
			{
				...
				int y = Integer.parseInt(args[1]);
				BoardComponent square = GameBoard.Instance().GetBoard().get(y).get(x);
				return new SpawnSheildCommand(square, args);
